...
public class SysAreaController extends AbstractController<SysAreaPo, SysAreaDto, SysAreaVo> {
    ...
    private void checkEntity(SysAreaDto entity) {
        ...
        if (CollectionUtil.isNotEmpty(list)) {
            AssertUtil.notNull(entity.getId(), "同一组区域编码不能重复");
            boolean b = list.stream().anyMatch(f -> !f.getId().equals(entity.getId()));
            AssertUtil.isFalse(b, "同一组区域编码不能重复");
        }
        q = new SysAreaPo();
        ...
        //同一父级下不能有相同的CODE
        if (CollectionUtil.isNotEmpty(list)) {
            AssertUtil.notNull(entity.getId(), "同一组区域名称不能重复");
            boolean b = list.stream().anyMatch(f -> !f.getId().equals(entity.getId()));
            AssertUtil.isFalse(b, "同一组区域名称不能重复");
        }
    }
