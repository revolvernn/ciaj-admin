...
public class SysPermissionController extends AbstractController<SysPermissionPo, SysPermissionDto, SysPermissionVo> {
    ...
    public ResponseEntity deleteFlag(@PathVariable("id") String id) {
        ...
        AssertUtil.isEmpty(select, "权限已绑定角色，不能删除。");
        //
    ...
    private void checkPermission(SysPermissionDto entity) {
        ...
        if (CollectionUtil.isNotEmpty(sysPermissions)) {
            AssertUtil.notNull(entity.getId(), "同一组权限名称不能重复");
            boolean b = sysPermissions.stream().anyMatch(f -> !f.getId().equals(entity.getId()));
            AssertUtil.isFalse(b, "同一组权限名称不能重复");
        }
        ...
        //同一父级下不能有相同的NAME
        if (CollectionUtil.isNotEmpty(sysPermissions)) {
            AssertUtil.notNull(entity.getId(), "同一组权限码不能重复");
            boolean b = sysPermissions.stream().anyMatch(f -> !f.getId().equals(entity.getId()));
            AssertUtil.isFalse(b, "同一组权限码不能重复");
        }
    ...
    private void updateParentIds(SysPermissionDto entity) {
        ...
        Safes.of(sysPermissions).forEach(po -> {
