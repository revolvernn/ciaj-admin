		...
		<el-dialog :close-on-click-modal="false":title="addOrUpdateForm.title"  :visible.sync="addOrUpdateForm.${jsName}FormVisible" width="400px">
           ...
           <#if tableClass.allFields??>
            <#list tableClass.allFields as field>
                <el-form-item label="<#if field.remarks??>${field.remarks}</#if>" prop="${field.fieldName}" :label-width="addOrUpdateForm.${jsName}FormLabelWidth" >
               		 <el-input v-model="addOrUpdateForm.${jsName}.${field.fieldName}" autocomplete="off" placeholder="<#if field.remarks??>${field.remarks}</#if>"></el-input>
               	</el-form-item>
            </#list>
            </#if>
			</el-form>
