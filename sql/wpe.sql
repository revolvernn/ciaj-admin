...
CREATE TABLE `boot_db`.`wpe_electrician_record` (
  ...
  `work_type` varchar(50) DEFAULT NULL COMMENT '工作方式',
...
DROP TABLE IF EXISTS `boot_db`.`wpe_final_statement`;

CREATE TABLE `boot_db`.`wpe_final_statement` (
  `id` varchar(32) NOT NULL COMMENT '主键',
  `user_id` varchar(32) NOT NULL COMMENT '用户ID',
  `day` datetime DEFAULT NULL COMMENT '结算日',
	`money` decimal(10,2) DEFAULT '0.00' COMMENT '款项金额',
  `remark` varchar(255) DEFAULT NULL COMMENT '备注',
  `create_at` varchar(32) DEFAULT NULL COMMENT '创建人',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_at` varchar(32) DEFAULT NULL COMMENT '更新人',
  ...
  `last_modify` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后更新时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='水电工结算单';